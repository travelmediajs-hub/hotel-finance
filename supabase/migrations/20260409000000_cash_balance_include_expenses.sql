-- Update property_cash_balances view to also deduct cash-paid expenses
-- and daily-report cash revenue, keeping existing withdrawals / collections /
-- received funds / in-transits logic.
...
SELECT
  ...
  pcr.opening_balance
    ...
    -- Expenses paid from this property's cash register
    - COALESCE((
      SELECT SUM(e.paid_amount)
      FROM expenses e
      WHERE e.property_id = pcr.property_id
        AND e.payment_method = 'CASH'
        AND e.status IN ('PAID', 'PARTIAL')
        AND e.paid_at IS NOT NULL
        AND e.paid_at >= pcr.opening_balance_date
    ), 0)
  AS current_balance
