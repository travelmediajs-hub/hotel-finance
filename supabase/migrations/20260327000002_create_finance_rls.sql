...
RETURNS boolean AS $$
  ...
  ) OR public.user_role() IN ('ADMIN_CO', 'FINANCE_CO')
$$ LANGUAGE sql SECURITY DEFINER STABLE;
...
RETURNS boolean AS $$
  ...
  OR public.user_role() IN ('ADMIN_CO', 'FINANCE_CO')
  OR (public.user_role() = 'MANAGER' AND EXISTS (
...
CREATE POLICY user_profiles_select ON user_profiles FOR SELECT
  USING (id = auth.uid() OR public.user_role() IN ('ADMIN_CO', 'FINANCE_CO'));
CREATE POLICY user_profiles_update ON user_profiles FOR UPDATE
  USING (id = auth.uid() OR public.user_role() = 'ADMIN_CO');
...
CREATE POLICY daily_reports_insert ON daily_reports FOR INSERT
  WITH CHECK (public.has_department_access(department_id) AND public.user_role() = 'DEPT_HEAD');
...
CREATE POLICY daily_report_lines_all ON daily_report_lines FOR ALL
  USING (EXISTS (
    ...
    WHERE dr.id = daily_report_id AND public.has_property_access(dr.property_id)
  ));
...
CREATE POLICY consolidations_manage ON property_consolidations FOR ALL
  USING (public.has_property_access(property_id)
    AND public.user_role() IN ('MANAGER', 'ADMIN_CO', 'FINANCE_CO'));
...
CREATE POLICY cash_collections_insert ON cash_collections FOR INSERT
  WITH CHECK (public.user_role() IN ('ADMIN_CO', 'FINANCE_CO'));
...
CREATE POLICY bank_accounts_co ON bank_accounts FOR ALL
  USING (public.user_role() IN ('ADMIN_CO', 'FINANCE_CO'));
